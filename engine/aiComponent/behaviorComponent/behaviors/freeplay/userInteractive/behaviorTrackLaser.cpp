...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void BehaviorTrackLaser::GetBehaviorOperationModifiers(BehaviorOperationModifiers& modifiers) const
{
  modifiers.behaviorAlwaysDelegates = false;
  modifiers.wantsToBeActivatedWhenOnCharger = false;
  // Request Lasers (dots) and Stats (image mean) while in activatable *and* active
  // scope. Without activatable Lasers, WantsToBeActivated never sees unconfirmed dots.
  modifiers.visionModesForActivatableScope->insert({ VisionMode::Lasers, EVisionUpdateFrequency::High });
  modifiers.visionModesForActivatableScope->insert({ VisionMode::Stats, EVisionUpdateFrequency::High });
  modifiers.visionModesForActiveScope->insert({ VisionMode::Lasers, EVisionUpdateFrequency::High });
  modifiers.visionModesForActiveScope->insert({ VisionMode::Stats, EVisionUpdateFrequency::High });
}
  ...
  switch( tag )
  {
    ...
    case MessageEngineToGameTag::RobotProcessedImage:
    {
      ...
      if(State::WaitingForExposureChange == _dVars.state && !exposureChangeAlreadyObserved)
      {
        ...
        {
          if(VisionMode::Stats == (*visionModeIter))
          {
...
void BehaviorTrackLaser::Cleanup()
{
  ...
  // Restore original params then re-enable AE and WB (SetAndDisableCameraControl disables both).
  ...
  GetBEI().GetVisionComponent().EnableWhiteBalance(true);
