"""Stock-firmware RedirectViz connect path.

Promoted from docs/mapping/viz-udp-poc/redirect.py. Uses the existing
Game-to-Engine path (no RedirectVizTo / WebViz :8888):
...
Helpers ANKICONN / bind_viz / local_ipv4s are duplicated from listen.py
because this branch does not have udp.py.
"""
...
# Copied from docs/mapping/viz-udp-poc/listen.py
VIZ_PORT = 5252
ANKICONN = b"ANKICONN"
MAX_MSG = 2848
...
def pack_ping(counter: int, time_sent_ms: float, is_response: bool) -> bytes:
    return bytes([G2E_PING]) + struct.pack(
        "<IdB", counter & 0xFFFFFFFF, time_sent_ms, 1 if is_response else 0
    )
