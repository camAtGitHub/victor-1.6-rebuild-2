...
Write-Host ""
Write-Host "After chrome shows LIVE / WORLD updates, enable the camera:"
Write-Host "  http://${robotArg}:8888/consolevars  ->  Vision.General.VisionModes  ->  Viz"
Write-Host "  curl `"http://${robotArg}:8888/consolevarset?key=Viz&value=true`""
Write-Host ""
Write-Host "Empty CAMERA text with a live WORLD is Viz mode off, not a Windows bind failure."
